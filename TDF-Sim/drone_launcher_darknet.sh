...
`# Gazebo DarkNet                                                                              ` \
`#---------------------------------------------------------------------------------------------` \
--tab --title "Gazebo DarkNet"  --command "bash -c \"
roslaunch darknet_ros darknet_ros_tdf.launch namespace_input:=$MAV_NAME$DRONE_SWARM_ID namespace_output:=drone$NUMID_DRONE;
exec bash\"" &
